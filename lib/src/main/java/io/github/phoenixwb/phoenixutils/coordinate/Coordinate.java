...
public class Coordinate implements Serializable {
	...
	public void rotate(Coordinate axis, boolean clockwise) {
	}
	
	public void placeRotated(Coordinate axis, boolean clockwise) {
	}
	...
	public Angle getRotation(Coordinate axis) {
		...
		if(ObjectUtil.isPositive(xDif) && ObjectUtil.isPositive(yDif)) {
			return new Angle(YPlane.NORTH, 90 - Math.atan(Math.abs(yDif) / Math.abs(xDif)), XPlane.EAST);
		}
		
		if(ObjectUtil.isNegative(xDif) && ObjectUtil.isPositive(yDif)) {
			return new Angle(YPlane.NORTH, Math.atan(Math.abs(yDif) / Math.abs(xDif)), XPlane.WEST);
		}
		
		if(ObjectUtil.isPositive(xDif) && ObjectUtil.isNegative(yDif)) {
			return new Angle(YPlane.SOUTH, Math.atan(Math.abs(yDif) / Math.abs(xDif)), XPlane.WEST);
		}
		
		return new Angle(YPlane.SOUTH, 90 - Math.atan(Math.abs(yDif) / Math.abs(xDif)), XPlane.EAST);
	}
}
